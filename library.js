...
const ERROR_REASON_ADVICE = {
  "拼写错误": "重点观察字母顺序、双写字母和词尾变化，并重新默写 2-3 遍。",
  "词义混淆": "重点对比核心含义、使用场景和同义替换。",
  "完全不会": "先看中文释义、例句和使用场景，再重新默写。",
  "搭配不会": "重点记住常用搭配和例句。",
  "发音不熟": "先跟读发音，再根据中文释义重新默写。",
  "一时想不起来": "缩短复习间隔，连续几天快速默写。",
};
const PAGE_SIZE = 20;
...
function wrongReasonCell(state) {
  ...
  const advice = ERROR_REASON_ADVICE[primaryReason] || "建议下次答错时选择错误原因，方便系统安排复习。";
...
function renderWrongReasonStats() {
  ...
  wrongWords.forEach((item) => {
    const marked = errorReasonsFor(wordState(item.id));
    if (!marked.length) unmarked += 1;
    marked.forEach((reason) => {
      if (reason in counts) counts[reason] += 1;
    });
  });
  els.wrongReasonStatGrid.innerHTML = wrongWords.length
    ? [["错词总数", wrongWords.length], ...reasons.map((reason) => [reason, counts[reason]]), ["未标记", unmarked]]
      .map(([label, count]) => `<div><span>${escapeHtml(label)}</span><strong>${count} 个</strong></div>`).join("")
    : "<p>暂无错词统计。完成默写并标记错误原因后，这里会自动生成分析结果。</p>";
}
...
function render() {
  ...
  els.wrongReasonStats.classList.toggle("hidden", view !== "wrong");
  if (view === "wrong") renderWrongReasonStats();
  ...
  if (!filtered.length) {
    els.libraryEmpty.innerHTML = view === "wrong"
      ? els.wrongReasonFilter.value !== "all"
        ? "<strong>当前分类下暂无错词</strong><span>可以选择其他错误原因，或继续完成新的默写练习。</span>"
